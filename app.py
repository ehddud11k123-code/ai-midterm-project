...
def get_text_input(label: str = "") -> str:
    ...
    if method == "텍스트 입력":
        ...
        with col1:
            st.button("📋 한국어 샘플", key=f"sample_ko_{label}", on_click=_load_sample_ko, use_container_width=True)
        with col2:
            st.button("📋 영어 샘플", key=f"sample_en_{label}", on_click=_load_sample_en, use_container_width=True)
    else:
...
with st.sidebar:
    st.title("📄 스마트 문서 분석기")
    st.caption("한국어·영어 자동 인식")
    st.divider()

    mode = st.radio("모드 선택", ["📄 논문 분석", "문서 분석"], key="mode")
    st.divider()

    input_text = get_text_input(label=mode)

    if mode == "📄 논문 분석":
        analyze_clicked = st.button(
            "🔍 논문 분석하기", type="primary",
            disabled=not input_text.strip(), use_container_width=True,
        )
    else:
        analyze_clicked = st.button(
            "🔍 분석하기", type="primary",
...
if mode == "📄 논문 분석":
    ...
    if analyze_clicked:
        with st.spinner("분석 중..."):
            ...
            st.session_state["doc_text"] = input_text
